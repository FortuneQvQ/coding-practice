...
namespace
{
// 检查文本是否为无 NUL、无异常控制符的有效 UTF-8；addNews 调用它阻止乱码入库。
...
// 在分类器规范来源和主题后执行质量校验与 INSERT OR IGNORE；
// 返回 true 仅表示本次真实新增，main 据此扣减每种子配额。
bool database::addNews(const News& news)
{
    ...
    if (rc != SQLITE_DONE) cerr << "[数据库] 插入失败：" << sqlite3_errmsg(db) << endl;
    return inserted;
}

// 批量重算历史记录的来源和主题，只更新变化行；main 在正常和只生成模式启动时调用。
...
// 读取种子的普通列表 URL 游标或动态接口页码，未保存时返回第一页默认值。
...
// 以 UPSERT 方式持久化续爬游标，供下一次运行跳过已经完成的历史分页。
...
// 查询某详情 URL 的连续失败次数，与 main 中的最大失败阈值配合。
...
// 记录或累加详情失败及原因，让临时故障可重试、永久坏链最终可放弃。
...
namespace
{
// 把 sqlite3_exec 返回的一行字段转换为 News 对象，供通用查询函数复用。
...
// 执行只读查询并收集 News 列表，为时间、标题、来源和主题查询提供公共实现。
